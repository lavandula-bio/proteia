...
const state = {
  ...
  proteinId: null,
  boxId: null,
  bitmaps: new Map(), // image id -> ImageBitmap of its preview
};
...
async function bitmapOf(imageId) {
  if (!state.bitmaps.has(imageId)) {
    const response = await request("GET", `/api/images/${imageId}/preview`);
    state.bitmaps.set(imageId, await createImageBitmap(await response.blob()));
  }
...
async function placeBox(x, y, { grow, clientX, clientY }, laneIndex = null) {
  if (!state.proteinId) {
    showStatus("Choose a protein on this image first.");
    return;
  }
  const body = { protein_id: state.proteinId, x, y, lane_index: laneIndex, grow };
  try {
    const answer = await call("POST", "/api/boxes", body);
    state.boxId = answer.band_id;
    showStatus("");
    applyProject(answer.project);
  } catch (error) {
    const why = LANE_QUESTIONS[error.code];
    if (why && laneIndex === null) {
      askLane(why, (lane) => placeBox(x, y, { grow, clientX, clientY }, lane), clientX, clientY);
    } else {
      report(error);
    }
  }
...
function askLane(message, onChoose, clientX, clientY) {
  const picker = $("lane-picker");
  const buttons = $("lane-picker-buttons");
  buttons.replaceChildren();
  const project = state.project;
  const taken = new Set(
    (project.proteins.find((p) => p.id === state.proteinId) || { bands: [] }).bands.map(
      (b) => b.lane_index,
    ),
  );
  for (const lane of project.lanes) {
  ...
  picker.style.left = `${Math.max(8, clientX - stage.left)}px`;
  picker.style.top = `${Math.max(8, clientY - stage.top)}px`;
  picker.hidden = false;
}
...
$("quit").addEventListener("click", async () => {
  ...
  try {
    ...
    if (!(error instanceof ApiError && error.status === 401)) {
      showStatus("Proteia did not stop. Try Quit again.");
      $("quit").disabled = false;
    }
  }
});
